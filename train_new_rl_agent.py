...
Updated to work with modern Gym/Gymnasium API.
"""
...
def train_dqn_agent(env: RLPoolEnv, 
    ...
    for episode in tqdm(range(num_episodes), desc="Training"):
        # Reset environment
        state, _ = env.reset()  # Get initial state and ignore info dict
        episode_reward = 0
        episode_losses = []
        done = False
        step = 0
        ...
        while not done:
            ...
            # Execute action in environment (using modern Gym/Gymnasium API)
...
def compare_models(env: RLPoolEnv, rl_agent: Any, save_dir: str):
    ...
    # Convert keys to list to ensure consistent ordering
    keys = list(rl_action_counts.keys())
    rl_values = [rl_action_counts[k] for k in keys]
    rule_values = [rule_action_counts[k] for k in keys]
